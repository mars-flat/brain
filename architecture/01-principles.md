...
3. **Graceful degradation.** If no seed clears the abstention gate (§5.5), `brain.recall` returns the **vault catalog** — every node as a one-line stub, read straight out of the index (`SELECT id, type, title FROM nodes`), capped to the budget with the truncation stated — and lets the model pick seeds itself. A frontier model reading a table of contents is a perfectly good retriever, and it needs no vector math. There is no `index.md` file; the catalog is a query, rendered on demand.

**The seam stays open.** `Embedder` is a port in `packages/contracts` and nothing implements it: the default wiring is *absent*, and every consumer treats absence as the normal case. No null embedder, no OpenAI embedder, no fusion switch exist — if recall proves insufficient after real use, the implementation and the fusion step are the work, not a config flip. Measure first (§8.5 has the eval harness, and W3's retrieval tuning is what the numbers actually asked for); add the model only if they demand it.
