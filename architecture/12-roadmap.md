...
| 10 | ~~**Tasks: a tab on the console, or a subdomain?**~~ **Confirmed 2026-09-24:** `/tasks` on the console, as built | ~~`/tasks` on the console~~ — as built; detail in `packages/tasks/README.md` |
| 11 | ~~**Tasks: interval required at creation?**~~ **Confirmed 2026-09-24:** required, with an explicit one-off, as built | ~~Required, with a one-off option~~ — as built |
| 12 | ~~**Tasks: a due-anchored task late by more than one interval**~~ **Confirmed 2026-09-18:** advance to the first future slot, keeping the cadence phase | ~~Advance to the first future slot~~ — as built |
| 13 | ~~**Tasks: which zone is "today"?**~~ **Decided 2026-09-18:** `TASKS_TZ` on the VM set to the owner's zone; public default UTC | ~~`TASKS_TZ` on the VM, UTC when unset~~ — set |
| 14 | ~~**Tasks: blocking completion page, or an inline reversible line?**~~ **Confirmed 2026-09-24:** the blocking "schedule again?" page, yes as default, as built | ~~The spec's page~~ — as built; the inline line stays a one-file swap |
