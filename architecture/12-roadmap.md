...
| 10 | ~~**Tasks (§16): a tab on the console, or a subdomain?**~~ **Confirmed by the owner 2026-09-24:** `/tasks` on the console, as built — a second subdomain is a cert + timer, a Caddy block, an Auth0 client + callback, a session secret, and a compose service, to deliver a tab | ~~`/tasks` on the console~~ — as built |
| 11 | ~~**Tasks: interval required at creation?**~~ **Confirmed by the owner 2026-09-24:** required at creation, with an explicit "one-off" option, as built | ~~Required, with an explicit "one-off" option~~ — as built |
| 12 | ~~**Tasks: a due-anchored task late by more than one interval**~~ **Confirmed by the owner 2026-09-18:** advance to the first future slot, keeping the cadence phase (a weekly Monday task done three weeks late is next due the following Monday) | ~~Advance to the first future slot~~ — as built |
| 13 | ~~**Tasks: which zone is "today"?**~~ **Decided (owner, 2026-09-18):** `TASKS_TZ` in the VM's compose `.env` is set to the owner's local zone and both containers were recreated with it; the public default stays UTC | ~~`TASKS_TZ` on the VM, UTC when unset~~ — set |
| 14 | ~~**Tasks: the completion prompt — the spec's blocking yes/no page, or an inline reversible line?**~~ **Confirmed by the owner 2026-09-24:** the blocking "schedule again?" page with yes as the default button, as built. The inline reversible line stays a one-file swap if the prompt ever wears thin | ~~The spec's page, yes as the default button~~ — as built |
