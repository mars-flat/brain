...
**Phases P0–P4 are complete. P5 is most of the way in (2026-08-27, one working day):**
...
- **Azure**: `rg-brain` + `brain-vm` exist (B2pls_v2 ARM, no public IP, 32 GiB data disk). The VM awaits an owner-run bootstrap (`deploy/vm/provision.sh` — secrets can't transit the agent's tool calls) — see `QUESTIONS-FOR-OWNER.md` P5-5.
- **Remaining for P5 done-when**: owner bootstrap (P5-5), Auth0 tenant wiring (§12 Q6 — needs P5-1 answers), `tailscale up` on the VM (P5-3), GHCR package visibility (P5-4), then a VM-sourced restore drill and a green end-to-end deploy.
