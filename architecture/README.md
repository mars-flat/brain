...
- **Remaining for P5 done-when — exactly one item**: the Auth0 tenant wiring (§12 Q6). Owner chose "script it" (`scripts/auth0-setup.ts` is ready); blocked on the `brain-mgmt` Management-API credential landing in `.env` (§13). After that: issuer swap on the VM, authed recall from the laptop over the tailnet, `install()` pointing the local harness at the remote gateway — and P5 closes. P6 (Discord) is **deferred by the owner** until they provide the bot token.
