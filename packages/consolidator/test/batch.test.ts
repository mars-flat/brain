/**
 * Batched consolidation (P5, §12 Q4): the cadence submits once, waits
 * without burning §5.7 retry attempts, consolidates when the batch lands,
 * and bounds a permanently failing episode at three submissions before
 * the normal dead-letter path takes it.
 */
...
class FakeBatchClient implements BatchModelClient {
  ...
  submitBatch(items: BatchItem[]): Promise<string> {
    ...
    this.submissions.push({ id, items });
...
describe("batched consolidation (§12 Q4)", () => {
  test("submits once, waits across many cycles without dead-lettering, consolidates when done", async () => {
    const { vault, episode, model, cycle } = await setup("we decided something durable");

    const first = await cycle();
    expect(first.submitted).toEqual([episode.episode_id]);
  ...
  test("a failing item resubmits at most three times, then dead-letters via §5.7", async () => {
    ...
    for (let round = 1; round <= 3; round++) {
      await cycle(); // submits batch_{round}
  ...
  test("redelivered episodes stay idempotent through the batch path", async () => {
    ...
    expect(again.submitted).toEqual([]);
    expect(model.submissions.length).toBe(1);
  });
});
